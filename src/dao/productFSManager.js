...
class productFSManager {
  ...
  constructor(file) {
    // Guardamos el archivo donde se van a persistir los productos.
    this.file = file;
  }

  // Devuelve todos los productos guardados en el archivo.
  ...
  async createProduct(product) {
    ...
    const newProduct = {
      id: this.getProductID(products), // Generamos un ID incremental.
      title,
      description,
      code,
      price,
      ...
      stock,
      category,
      thumbnails: thumbnails ?? [] // Si no hay imágenes, se guarda un array vacío.
    };
  ...
  // Genera un ID nuevo para el producto, sumando 1 al último existente.
  ...
  async updateProduct(pid, productUpdate) {
    ...
    if (productFilter.length > 0) {
      // Actualizamos solo los campos que vinieron en la petición.
